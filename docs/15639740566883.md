...
这时，kubelet 就会**调用 CRI 的 Exec 接口**，而负责响应这个接口的，自然就是具体的 **CRI shim**。

但在这一步，CRI shim 并不会直接去调用后端的容器项目（比如 Docker ）来进行处理，而只会**返回一个 URL 给 kubelet**。**这个 URL，就是该 CRI shim 对应的 Streaming Server 的地址和端口。**
